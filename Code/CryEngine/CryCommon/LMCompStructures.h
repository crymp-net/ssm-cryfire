...
struct RenderLMData : public _reference_target_t
{
	...
	RenderLMData(struct IRenderer *pIRenderer,/* int iColorLerpTex, int iHDRColorLerpTex, int iDomDirectionTex, int iOcclusionTex = 0,*/ int iRAETex = 0 )
	{
...
/*		m_iColorLerpTex		= iColorLerpTex;
		...
		m_iOcclusionTex = iOcclusionTex;*/
		m_iRAETex = iRAETex;
	};

	//! \brief Obtain textures
/*	int GetColorLerpTex() const		{ return m_iColorLerpTex; };
	int GetHDRColorLerpTex() const		{ return m_iHDRColorLerpTex; };
	int GetDomDirectionTex() const	{ return m_iDomDirectionTex; };
	int GetOcclusionTex() const { return m_iOcclusionTex; }*/
	int GetRAETex() const { return m_iRAETex; }

protected:
	//! \brief Destrucktor protected, call Release()
	~RenderLMData() 
	{ 
/*		if(m_iColorLerpTex != 0)
			m_pIRenderer->RemoveTexture(m_iColorLerpTex);
		if(m_iDomDirectionTex != 0)
			m_pIRenderer->RemoveTexture(m_iDomDirectionTex);
		if(m_iHDRColorLerpTex != 0)
			m_pIRenderer->RemoveTexture(m_iHDRColorLerpTex);

		if( 0 != m_iOcclusionTex )
			m_pIRenderer->RemoveTexture( m_iOcclusionTex );
*/
		if( 0 != m_iRAETex )
			m_pIRenderer->RemoveTexture( m_iRAETex );
	};
...
/*	int m_iColorLerpTex;            //!< Color + lerp factor texture
	...
	int m_iOcclusionTex;						//!< occlusion map texture*/
	int m_iRAETex;									//!< RAE texture
};
...
struct LMFalseLightFileHeader
{
	LMFalseLightFileHeader()
	{
		iVersion = 1;
		iNumObjects = 0;  
	};
...
//! \brief Name of the FalseLight data file in the level's directory
#define LM_FALSE_LIGHTS_EXPORT_FILE_NAME "FalseLights.dat"
